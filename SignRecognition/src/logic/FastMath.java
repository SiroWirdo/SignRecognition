...
public class FastMath {
	...
	private static final double SAFE_MIN = Double
			.longBitsToDouble((EXPONENT_OFFSET - 1022l) << 52);
	...
	private static double doubleHighPart(double d) {
		...
		long xl = Double.doubleToRawLongBits(d); // can take raw bits because
		// just gonna convert it
		// back
	...
	private static double exp(double x, double extra, double[] hiPrec) {
		...
		if (x < 0.0) {
			...
			// We don't check against intVal here as conversion of large
			// negative double values
			// may be affected by a JIT bug. Subsequent comparisons can safely
			// use intVal
			if (x < -746d) {
				if (hiPrec != null) {
					hiPrec[0] = 0.0;
		...
		intPartA = 0;// ExpIntTable.EXP_INT_TABLE_A[EXP_INT_TABLE_MAX_INDEX+intVal];
		intPartB = 0;// ExpIntTable.EXP_INT_TABLE_B[EXP_INT_TABLE_MAX_INDEX+intVal];
		...
		final double fracPartA = 0;// ExpFracTable.EXP_FRAC_TABLE_A[intFrac];
		final double fracPartB = 0;// ExpFracTable.EXP_FRAC_TABLE_B[intFrac];
		...
		// If tempC is positive infinite, the evaluation below could result in
		// NaN,
	...
	private static double expm1(double x, double hiPrecOut[]) {
...

		{
			...
			double tempA = 1;// ExpFracTable.EXP_FRAC_TABLE_A[intFrac] - 1.0;
			double tempB = 1;// ExpFracTable.EXP_FRAC_TABLE_B[intFrac];
	...
	private static double log(final double x, final double[] hiPrec) {
		...
		// ln(2)
		final double[] lnm = null;// ?//lnMant.LN_MANT[(int)((bits &
		// 0x000ffc0000000000L) >> 42)];
	...
	public static double pow(final double x, final double y) {
		...
		if (y == 0) {
			...
			if (yRawExp > 1085) {
				// y is either a very large integral value that does not fit in
				// a long or it is a special number
				...
				if ((yRawExp == 2047 && yRawMantissa != 0)
					...
					// the absolute value of x is either greater or smaller than
					// 1.0

					// if yRawExp == 2047 and mantissa is 0, y = -infinity or y
					// = +infinity
					// if 1085 < yRawExp < 2047, y is simply a large number,
					// however, due to limited
					// accuracy, at this magnitude it behaves just like infinity
					// with regards to x
					if ((y > 0) ^ (xRawExp < 1023)) {
						// either y = +infinity (or large engouh) and abs(x) >
						// 1.0
				...
				if (yRawExp >= 1023) {
					// y may be an integral value, which should be handled
					// specifically
					final long yFullMantissa = IMPLICIT_HIGH_BIT | yRawMantissa;
					if (yRawExp < 1075) {
						// normal number with negative shift that may have a
						// fractional part
						final long integralMask = (-1L) << (1075 - yRawExp);
						if ((yFullMantissa & integralMask) == yFullMantissa) {
							// all fractional bits are 0, the number is really
							// integral
							final long l = yFullMantissa >> (1075 - yRawExp);
						return FastMath.pow(x, (y < 0) ? -l : l);
						}
					} else {
						// normal number with positive shift, always an integral
						// value
						// we know it fits in a primitive long because yRawExp >
						// 1085 has been handled above
				...
				if (x == 0) {
					...
					// this is the general case, for regular fractional numbers
					// x and y
					...
					if (Double.isInfinite(lores)) { // don't allow this to be
						// converted to NaN
						return lores;
					}
	...
	/**
	 * Class operator on double numbers split into one 26 bits number and one 27
	 * bits number.
	 */
	private static class Split {
		...
		Split(final double high, final double low) {
			this(
					high == 0.0 ? (low == 0.0
					&& Double.doubleToRawLongBits(high) == Long.MIN_VALUE /*
					 * negative
					 * zero
					 */? -0.0
							 : low)
		...
		public Split multiply(final Split b) {
			...
			final double mulError = low
					* b.low
					- (((mulBasic.full - high * b.high) - low * b.high) - high
							* b.low);
		...
		public Split reciprocal() {
			...
			// if 1.0/d were computed perfectly, remultiplying it by d should
			// give 1.0
			// we want to estimate the error so we can fix the low order bits of
			// approximateInvLow
			// beware the following expressions must NOT be simplified, they
			// rely on floating point arithmetic properties
			...
			return Double.isNaN(error) ? splitInv : new Split(splitInv.high,
					splitInv.low - error / full);
		 ...
		 *
		 * @param e
		 *            exponent (beware, here it MUST be > 0; the only exclusion
		 *            is Long.MIN_VALUE)
	 ...
	 * @since 3.4
	 */
	public static int toIntExact(final long n) {// throws
	...
	public static int decrementExact(final int n) {// throws
		...
		// throw new
		// MathArithmeticException(LocalizedFormats.OVERFLOW_IN_SUBTRACTION, n,
		// 1);
		// }
	...
	public static int subtractExact(final int a, final int b) {
		...
		// if ((a ^ b) < 0 && (sub ^ b) >= 0) {
		// throw new
		// MathArithmeticException(LocalizedFormats.OVERFLOW_IN_SUBTRACTION, a,
		// b);
		// }
	...
	public static int multiplyExact(final int a, final int b) {
		...
		// throw new
		// MathArithmeticException(LocalizedFormats.OVERFLOW_IN_MULTIPLICATION,
		// a, b);
		// }
		return a * b;
	}
	...
	/**
	 * Finds q such that a = q b + r with 0 <= r < b if b > 0 and b < r <= 0 if
	 * b < 0.
	 * <p>
	 * This methods returns the same value as integer division when a and b are
	 * same signs, but returns a different value when they are opposite (i.e. q
	 * is negative).
	 * </p>
	 *
	 * @param a
	...
	/**
	 * Finds r such that a = q b + r with 0 <= r < b if b > 0 and b < r <= 0 if
	 * b < 0.
	 * <p>
	 * This methods returns the same value as integer modulo when a and b are
	 * same signs, but returns a different value when they are opposite (i.e. q
	 * is negative).
	 * </p>
	 *
	...
	public static double copySign(double magnitude, double sign) {
		...
		final long m = Double.doubleToRawLongBits(magnitude); // don't care
		// about NaN
	...
	public static int getExponent(final float f) {
		// NaN and Infinite will return the same exponent anywho so can use raw
		// bits
